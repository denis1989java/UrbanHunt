...
class APIService {
    ...
    func getChallenge(id: String) async throws -> Challenge {
        print("🔄 APIService.getChallenge called for id: \(id)")

        // Try with token first
        let token = try? await getFirebaseToken()

        let url = URL(string: "\(baseURL)/challenges/\(id)")!
        var request = URLRequest(url: url)
        request.httpMethod = "GET"

        // Add token if available (but endpoint works without it)
        if let token = token {
            request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
        }
    ...
    func confirmPrize(challengeId: String, message: String?, contentUrl: String?) async throws -> PrizeConfirmation {
        ...
        guard let url = URL(string: "\(Config.apiBaseURL)/api/prize-confirmations/challenge/\(challengeId)/confirm") else {
            throw APIError.invalidResponse
        }
